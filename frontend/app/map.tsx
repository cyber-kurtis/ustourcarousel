...
// WebView sadece native build'de import edilir — web'de iframe kullanıyoruz
...
// Native için inline HTML (web için /map-page.html static dosyası kullanılır)
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .leaflet-popup-content-wrapper{border-radius:14px;box-shadow:0 6px 24px rgba(0,0,0,.18);padding:0;overflow:hidden}
    .leaflet-popup-content{margin:0;width:240px!important}
    .popup-inner{padding:16px}
    .popup-name{font-size:14px;font-weight:700;color:#003580;margin-bottom:5px;line-height:1.3}
    .popup-desc{font-size:12px;color:#6E6E73;margin-bottom:14px;line-height:1.4}
    .popup-btn{background:#003580;color:#fff;border:none;padding:11px 0;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;width:100%}
    #overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:9999;align-items:flex-end;justify-content:center}
    #overlay.show{display:flex}
    #dialog{background:#fff;border-radius:20px 20px 0 0;padding:24px 20px 36px;width:100%;max-width:480px}
    #dialog-title{font-size:16px;font-weight:700;color:#1C1C1E;margin-bottom:6px;text-align:center}
    #dialog-sub{font-size:13px;color:#6E6E73;text-align:center;margin-bottom:20px}
    .dialog-btn{width:100%;padding:14px;border-radius:12px;border:none;font-size:15px;font-weight:700;cursor:pointer;margin-bottom:10px;display:block}
    .btn-yes{background:#003580;color:#fff}
    .btn-no{background:#F2F2F7;color:#003580}
  </style>
</head>
<body>
  <div id="map"></div>
  ...
  <script>
    ...
    var pinIcon=L.divIcon({html:'<div style="background:#003580;width:14px;height:14px;border-radius:50%;border:2.5px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,.4)"></div>',className:'',iconSize:[14,14],iconAnchor:[7,7]});
    var locs=[
      {name:"Belgrad — Kalemegdan Kalesi",lat:44.8215,lng:20.4513,desc:"Panoramik tur başlangıç & buluşma noktası"},
    ...
    function confirmNo(){pendingUrl=null;document.getElementById('overlay').classList.remove('show');}
  </script>
</body>
</html>`;
...
export default function MapScreen() {
  ...
  const mapContent =
    Platform.OS === "web" ? (
      // Web: statik HTML dosyasını iframe ile yükle
      // @ts-ignore
      <iframe
        src="/map-page.html"
        style={{
        ...
        title="Konumlar Haritası"
        allow="geolocation"
      />
    ) : (
      // Native: inline HTML ile WebView
      <WebView
        ref={webViewRef}
